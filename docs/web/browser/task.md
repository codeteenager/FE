...
1. 那么微任务是怎么产生的呢？在现代浏览器里面，产生微任务有两种方式。
...
|     |  宏任务   |   微任务  |
| ----- | ----- | ------ |
| 相应的方法事件 | script、setTimeout/setInterval、UI rendering/UI事件、postMessage、MessageChannel、setImmediate(Node.js) | Prmise、MutationObserver、Object.observe(Proxy对象替代)、process.nextTick(Node.js) |
| 运行顺序 | 后运行 | 先运行 |
| 是否触发新一轮tick | 会 | 不会 |
